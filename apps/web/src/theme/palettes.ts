/**
 * The built-in theme library, carried over from T3 Code unchanged.
 *
 * A theme is a set of product colour *roles* (not CSS variables): the same
 * palette therefore drives the canvas, the panels and the settings surfaces
 * without any of them knowing which theme is installed. `theme/useTheme.ts`
 * writes the resolved role values onto `html` as `--app-theme-*` custom
 * properties, and `index.css` maps those onto the semantic token layer.
 *
 * Values are verbatim T3 Code (`packages/shared/src/themePalettes.ts`), so a
 * theme file exported from either app renders identically in the other.
 */
...
/**
 * The palette Observer wears with no theme installed, captured from the app's
 ...
 * their real backdrops because theme colours are stored opaque.
 */
...
export const T3_CODE_THEME_ID = "t3-code" as const;
export const T3_CODE_THEME_LABEL = "T3 Code";

/** The stock look as a theme definition, for the library card and for seeding. */
export const T3_CODE_THEME: ThemeDefinition = {
  id: T3_CODE_THEME_ID,
  label: T3_CODE_THEME_LABEL,
  appearance: "light",
  colors: T3_CODE_LIGHT_THEME_COLORS,
  variants: { dark: T3_CODE_DARK_THEME_COLORS },
};
