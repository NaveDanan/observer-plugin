...
import { behaviorDirective, employeeDescription, getEmployee, activeEmployees, canonicalEmployeeId } from "@observer-ai/roster"
import { opencodeAgentDir } from "./paths.js"

/**
 * Generated OpenCode definitions cover six default employees and enabled
 * optional specialists. Seat control only adds a model and variant.
 *
 ...
 * its optional model pins. Every enabled employee gets a selectable agent.
...
function renderDesired(seats: SeatsConfig, notes: string[], control: boolean): Map<string, string> {
  ...
  const blocked = new Set(
    issues.filter((issue) => issue.severity === "error" && issue.employeeId).map((issue) => canonicalEmployeeId(issue.employeeId as string)),
  )
  ...
  for (const profile of activeEmployees(seats)) {
...
function opencodeSeats(seats: SeatsConfig, notes: string[]): OpencodeSeat[] {
  ...
  for (const { id } of activeEmployees(seats)) {
...
function renderAgent(employeeId: string, seats: SeatsConfig, target: OpencodeSeatTarget | undefined): string {
  ...
  const lines = [
    ...
    `description: ${yaml(employeeDescription(profile))}`,
    "mode: subagent",
  ...
  lines.push("---", "", behaviorDirective(applySeatSkills(profile, seats)), "", employeeRosterBriefing(seats), "")
  return lines.join("\n")
}

/** The exact native agent types and strengths every employee can delegate to. */
function employeeRosterBriefing(seats: SeatsConfig): string {
  return [
    "## Observer employee roster",
    "Only when the assignment authorizes nested delegation and the host supports it, use agent_spawn with these exact registered types. Handoff recommendations alone do not authorize spawning:",
    ...activeEmployees(seats).map(
      (profile) =>
        `- \`observer-${profile.id}\`: ${employeeDescription(profile)}`,
    ),
  ].join("\n")
}
